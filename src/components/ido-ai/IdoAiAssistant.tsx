...
export function IdoAiWorkspace({ children }: { children: ReactNode }) {
  ...
  const sendMutation = useMutation({
    mutationFn: (content: string) => sendIdoAiMessage(workspace.id, state.conversationId, content),
    onSuccess: async () => { setComposer(''); setAnswer(''); await queryClient.invalidateQueries({ queryKey: ['ido-ai', workspace.id] }) },
  })
  ...
  return <div className={`ido-ai-workspace${open ? ' is-agent-open' : ''}`}>
    ...
    <aside className="ido-ai-panel" ref={panelRef} aria-label="I Do AI assistant" aria-hidden={!open} tabIndex={-1}>
      ...
      <div className="ido-ai-scroll" ref={scrollRef} aria-live="polite">
        ...
        {state.messages.map((message) => <article className={`ido-ai-message is-${message.role}`} key={message.id}>{message.role === 'assistant' && <span className="ido-ai-message-mark"><SparkleMark /></span>}<div><strong>{message.role === 'assistant' ? 'I Do AI' : 'You'}</strong><p>{message.body}</p></div></article>)}
        ...
        {state.job && ['queued', 'running', 'failed'].includes(state.job.status) && <div className={`ido-ai-job is-${state.job.status}`}><span className="ido-ai-job-icon">{state.job.status === 'failed' ? '!' : <span className="ido-ai-spinner" />}</span><span><strong>{state.job.label}</strong><small>{state.job.detail}</small></span></div>}
        ...
        {activeBatch && <section className="ido-ai-batch" aria-labelledby="ido-ai-batch-title"><div className="ido-ai-batch-heading"><span>Proposed actions</span><strong id="ido-ai-batch-title">{activeBatch.summary}</strong></div>{activeBatch.actions.map((action) => <article className={`ido-ai-action is-${action.status}`} key={action.id}><div className="ido-ai-action-top"><strong>{action.title}</strong><span>{action.destination}</span></div><p>{action.description}</p>{action.status !== 'proposed' && <div className="ido-ai-decision"><Check size={13} /> {action.status}</div>}</article>)}{activeBatch.status === 'proposed' && <div className="ido-ai-batch-actions"><button type="button" disabled={reviewMutation.isPending} onClick={() => reviewMutation.mutate({ batchId: activeBatch.id, decision: 'reject' })}>Reject batch</button><button type="button" disabled={reviewMutation.isPending} onClick={() => reviewMutation.mutate({ batchId: activeBatch.id, decision: 'approve' })}>Approve batch</button></div>}{reviewMutation.error && <p className="ido-ai-error">{reviewMutation.error.message}</p>}</section>}
      </div>
      <form className="ido-ai-composer" onSubmit={(event: FormEvent) => { event.preventDefault(); submitMessage(composer) }}><label className="sr-only" htmlFor="ido-ai-message">Message I Do AI</label><textarea id="ido-ai-message" rows={2} value={composer} onChange={(event) => setComposer(event.target.value)} placeholder="Ask I Do AI anything..." /><div><span>I Do AI will ask before making changes.</span><button type="submit" disabled={!composer.trim() || sendMutation.isPending || isPreview} aria-label="Send message">↑</button></div></form>
    </aside>
  </div>
}
