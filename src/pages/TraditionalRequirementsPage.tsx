...
import { CheckCircle2, CircleDollarSign, PackageCheck, Plus, Truck, X } from 'lucide-react'
...
type Requirement = { id: string; item: string; category: string; quantity: number; unit: string; owner: string; supplier: string; estimated: number; actual: number; paid: number; due: string; recipient: string; status: RequirementStatus }
...
export function TraditionalRequirementsPage() {
  ...
  const requirementsQuery = useQuery({
    ...
    queryFn: async () => {
      const [{ data: ceremony, error: ceremonyError }, { data: requirements, error: requirementsError }] = await Promise.all([
        ...
        supabase!.from('traditional_requirements').select('id,item_name,category,required_quantity,unit,responsible_party,estimated_minor,actual_minor,due_date,status,delivery_recipient,notes,traditional_requirement_payments(amount_minor)').eq('workspace_id', workspace.id).is('deleted_at', null).order('created_at'),
      ])
  ...
  const requirementMutation = useMutation({
    mutationFn: async (operation: { type: 'add'; item: Requirement } | { type: 'status'; id: string; status: RequirementStatus }) => {
  ...
  return <div className="page logistics-page">
    <header className="page-header"><div><p className="eyebrow">Traditional ceremony</p><h1>Requirements</h1><p className="page-lead">Itemise what is required, who is sourcing it, what it costs, and whether it has been delivered and accepted.</p></div><button className="button primary" type="button" onClick={() => setAdding(true)}><Plus size={15} /> Add requirement</button></header>
    ...
    {adding && <RequirementForm onClose={() => setAdding(false)} onAdd={(item) => { if (isPreview) setItems((current) => [item, ...current]); else requirementMutation.mutate({ type: 'add', item }); setAdding(false) }} />}
...
function RequirementForm({ onClose, onAdd }: { onClose: () => void; onAdd: (item: Requirement) => void }) {
  const [draft, setDraft] = useState({ item: '', category: '', quantity: '1', unit: 'pieces', owner: '', supplier: '', estimated: '', actual: '', paid: '', due: '', recipient: '' })
  const set = (key: keyof typeof draft, value: string) => setDraft((current) => ({ ...current, [key]: value }))
  function submit(event: FormEvent) { event.preventDefault(); if (!draft.item.trim()) return; onAdd({ id: crypto.randomUUID(), item: draft.item.trim(), category: draft.category.trim(), quantity: Number(draft.quantity) || 1, unit: draft.unit.trim(), owner: draft.owner.trim(), supplier: draft.supplier.trim(), estimated: Number(draft.estimated) || 0, actual: Number(draft.actual) || 0, paid: Number(draft.paid) || 0, due: draft.due, recipient: draft.recipient.trim(), status: 'required' }) }
  return <section className="logistics-entry"><header><div><p className="eyebrow">Traditional list</p><h2>New requirement</h2></div><button type="button" onClick={onClose} aria-label="Close"><X size={17} /></button></header><form onSubmit={submit}><div className="logistics-fields"><Field label="Item"><input value={draft.item} onChange={(e) => set('item', e.target.value)} required /></Field><Field label="Category"><input value={draft.category} onChange={(e) => set('category', e.target.value)} /></Field><Field label="Quantity"><input type="number" min="1" value={draft.quantity} onChange={(e) => set('quantity', e.target.value)} /></Field><Field label="Unit"><input value={draft.unit} onChange={(e) => set('unit', e.target.value)} /></Field><Field label="Sourcing owner"><input value={draft.owner} onChange={(e) => set('owner', e.target.value)} /></Field><Field label="Supplier"><input value={draft.supplier} onChange={(e) => set('supplier', e.target.value)} /></Field><Field label="Estimated NGN"><input type="number" min="0" value={draft.estimated} onChange={(e) => set('estimated', e.target.value)} /></Field><Field label="Actual NGN"><input type="number" min="0" value={draft.actual} onChange={(e) => set('actual', e.target.value)} /></Field><Field label="Paid NGN"><input type="number" min="0" value={draft.paid} onChange={(e) => set('paid', e.target.value)} /></Field><Field label="Due date"><input type="date" value={draft.due} onChange={(e) => set('due', e.target.value)} /></Field><Field label="Delivery recipient"><input value={draft.recipient} onChange={(e) => set('recipient', e.target.value)} /></Field></div><footer><button className="button secondary" type="button" onClick={onClose}>Cancel</button><button className="button primary" type="submit">Save requirement</button></footer></form></section>
}
