...
import { CheckCircle2, CircleDollarSign, PackageCheck, Pencil, Plus, Truck, X } from '../components/Icon'
...
function RequirementForm({ initial, ceremonies, saving, onClose, onSave }: { initial?: Requirement; ceremonies: CeremonyOption[]; saving: boolean; onClose: () => void; onSave: (item: Requirement) => void }) {
  ...
  return <section className="logistics-entry"><header><div><p className="eyebrow">Ceremony list</p><h2>{initial ? 'Edit requirement' : 'New requirement'}</h2></div><button type="button" onClick={onClose} aria-label="Close"><X size={17} /></button></header><form onSubmit={submit}><div className="logistics-fields"><Field label="Ceremony"><Select required aria-label="Ceremony" value={draft.ceremonyId} onChange={(next) => set('ceremonyId', next)} options={ceremonies.map((ceremony) => ({ value: ceremony.id, label: ceremonyLabel(ceremony) }))} /></Field><Field label="Item"><input value={draft.item} onChange={(e) => set('item', e.target.value)} required /></Field><Field label="Category"><input value={draft.category} onChange={(e) => set('category', e.target.value)} /></Field><Field label="Quantity"><input type="number" min="1" value={draft.quantity} onChange={(e) => set('quantity', e.target.value)} /></Field><Field label="Unit"><input value={draft.unit} onChange={(e) => set('unit', e.target.value)} /></Field><Field label="Sourcing owner"><input value={draft.owner} onChange={(e) => set('owner', e.target.value)} /></Field><Field label="Supplier"><input value={draft.supplier} onChange={(e) => set('supplier', e.target.value)} /></Field><Field label="Estimated NGN"><input type="number" min="0" value={draft.estimated} onChange={(e) => set('estimated', e.target.value)} /></Field><Field label="Actual NGN"><input type="number" min="0" value={draft.actual} onChange={(e) => set('actual', e.target.value)} /></Field><Field label="Paid NGN"><input type="number" min="0" value={draft.paid} onChange={(e) => set('paid', e.target.value)} /></Field><Field label="Due date"><DateField aria-label="Due date" value={draft.due} onChange={(next) => set('due', next)} /></Field><Field label="Delivery recipient"><input value={draft.recipient} onChange={(e) => set('recipient', e.target.value)} /></Field><Field label="Status"><Select aria-label="Status" value={draft.status} onChange={(next) => set('status', next)} options={[{ value: 'required', label: 'Required' }, { value: 'ordered', label: 'Ordered' }, { value: 'sourced', label: 'Sourced' }, { value: 'delivered', label: 'Delivered' }, { value: 'approved', label: 'Approved' }]} /></Field></div><footer><Button variant="secondary" type="button" onClick={onClose}>Cancel</Button><Button variant="primary" type="submit" disabled={saving}>{saving ? 'Saving...' : initial ? 'Save changes' : 'Add requirement'}</Button></footer></form></section>
}
