...
function StockForm({ initial, groups, saving, onClose, onAdd }: { initial?: StockItem; groups: string[]; saving: boolean; onClose: () => void; onAdd: (item: StockItem) => void }) {
  ...
  return <EntryPanel title={initial ? 'Edit inventory' : 'Add inventory'} editing={Boolean(initial)} saving={saving} onClose={onClose} onSubmit={submit}><Field label="Fabric or item"><input value={name} onChange={(e) => setName(e.target.value)} required placeholder="e.g. Aso-ebi lace" /></Field><Field label="Group"><Select aria-label="Group" value={group} onChange={setGroup} options={groups.map((value) => ({ value, label: value }))} /></Field><Field label="Unit"><input value={unit} onChange={(e) => setUnit(e.target.value)} list="attire-units" required /><datalist id="attire-units">{units.map((value) => <option value={value} key={value} />)}</datalist></Field><Field label="Ordered"><input type="number" min="0" step="any" value={ordered} onChange={(e) => setOrdered(e.target.value)} /></Field><Field label="Received"><input type="number" min="0" step="any" value={received} onChange={(e) => setReceived(e.target.value)} /></Field><Field label="Unit cost NGN"><input type="number" min="0" step="0.01" value={unitCost} onChange={(e) => setUnitCost(e.target.value)} /></Field><Field label="Selling price NGN"><input type="number" min="0" step="0.01" value={unitPrice} onChange={(e) => setUnitPrice(e.target.value)} /></Field></EntryPanel>
}
