...
import { Boxes, CircleDollarSign, Pencil, Plus, Shirt, Truck } from '../components/Icon'
        ...
        <section className="record-table">
          ...
          {orders.length ? orders.map((order) => (
            <article className="attire-order-row" key={order.id}>
              ...
              <Select compact disabled={attireMutation.isPending} aria-label="Order status" value={order.status} onChange={(next) => { const status = next as OrderStatus; setOrders((current) => current.map((item) => item.id === order.id ? { ...item, status } : item)); if (!isPreview) attireMutation.mutate({ type: 'status', id: order.id, status }) }} options={[{ value: 'ordered', label: 'Ordered' }, { value: 'fitting', label: 'Fitting' }, { value: 'ready', label: 'Ready' }, { value: 'collected', label: 'Collected' }]} />
...
function OrderForm({ initial, items, groups, ceremonies, saving, onClose, onAdd }: { initial?: AttireOrder; items: StockItem[]; groups: string[]; ceremonies: Ceremony[]; saving: boolean; onClose: () => void; onAdd: (order: AttireOrder) => void }) {
  ...
  return <EntryPanel title={initial ? 'Edit attire order' : 'New attire order'} editing={Boolean(initial)} saving={saving} onClose={onClose} onSubmit={submit}><Field label="Recipient"><input value={recipient} onChange={(e) => setRecipient(e.target.value)} required placeholder="Who it is for" /></Field><Field label="Group"><Select aria-label="Group" value={group} onChange={setGroup} options={groups.map((value) => ({ value, label: value }))} /></Field><Field label="Ceremony"><Select required aria-label="Ceremony" placeholder="Select ceremony" value={event} onChange={setEvent} options={ceremonies.map((ceremony) => ({ value: ceremony.id, label: ceremony.kind }))} /></Field>{group === 'Couple' && <Field label="Look name"><input value={look} onChange={(e) => setLook(e.target.value)} placeholder="Ceremony, reception..." /></Field>}<Field label="Item"><input value={item} onChange={(e) => setItem(e.target.value)} list="stock-items" required placeholder="e.g. Agbada, gele" /><datalist id="stock-items">{items.filter((value) => value.group === group).map((value) => <option value={value.name} key={value.id} />)}</datalist></Field><Field label="Quantity"><input type="number" min="0.001" step="any" value={quantity} onChange={(e) => setQuantity(e.target.value)} /></Field><Field label="Total NGN"><MoneyInput prefix="NGN" value={total} onChange={setTotal} placeholder="0.00" /></Field><Field label="Paid NGN"><MoneyInput prefix="NGN" value={paid} onChange={setPaid} placeholder="0.00" /></Field><Field label="Tailor / designer"><input value={tailor} onChange={(e) => setTailor(e.target.value)} placeholder="Who is making it" /></Field><Field label="Status"><Select aria-label="Status" value={status} onChange={(next) => setStatus(next as OrderStatus)} options={[{ value: 'ordered', label: 'Ordered' }, { value: 'fitting', label: 'Fitting' }, { value: 'ready', label: 'Ready' }, { value: 'collected', label: 'Collected' }]} /></Field></EntryPanel>
}
...
function StockForm({ initial, groups, saving, onClose, onAdd }: { initial?: StockItem; groups: string[]; saving: boolean; onClose: () => void; onAdd: (item: StockItem) => void }) {
  ...
  return <EntryPanel title={initial ? 'Edit inventory' : 'Add inventory'} editing={Boolean(initial)} saving={saving} onClose={onClose} onSubmit={submit}><Field label="Fabric or item"><input value={name} onChange={(e) => setName(e.target.value)} required placeholder="e.g. Aso-ebi lace" /></Field><Field label="Group"><Select aria-label="Group" value={group} onChange={setGroup} options={groups.map((value) => ({ value, label: value }))} /></Field><Field label="Unit"><input value={unit} onChange={(e) => setUnit(e.target.value)} list="attire-units" required /><datalist id="attire-units">{units.map((value) => <option value={value} key={value} />)}</datalist></Field><Field label="Ordered"><input type="number" min="0" step="any" value={ordered} onChange={(e) => setOrdered(e.target.value)} /></Field><Field label="Received"><input type="number" min="0" step="any" value={received} onChange={(e) => setReceived(e.target.value)} /></Field><Field label="Unit cost NGN"><MoneyInput prefix="NGN" value={unitCost} onChange={setUnitCost} placeholder="0.00" /></Field><Field label="Selling price NGN"><MoneyInput prefix="NGN" value={unitPrice} onChange={setUnitPrice} placeholder="0.00" /></Field></EntryPanel>
}
...
function EntryPanel({ title, editing = false, saving, onClose, onSubmit, children }: { title: string; editing?: boolean; saving: boolean; onClose: () => void; onSubmit: (event: FormEvent) => void; children: React.ReactNode }) {
  return <Modal
    ...
    footer={<>
      ...
      <Button variant="primary" type="submit" form="logistics-form" disabled={saving}>{saving ? 'Saving...' : editing ? 'Save changes' : 'Save record'}</Button>
    </>}
  >
    <form id="logistics-form" onSubmit={onSubmit}><div className="logistics-fields">{children}</div></form>
  </Modal>
}
