...
export function SettingsPage() {
  ...
  return <div className="page settings-page">
    ...
    <section className="settings-section"><header><div><p className="eyebrow">General</p><h2>Wedding office</h2></div></header><div className="settings-fields"><label>Workspace name<input value={name} onChange={(event) => setName(event.target.value)} /></label><label>Reporting currency<select value={currency} onChange={(event) => setCurrency(event.target.value)}><option>NGN</option><option>GBP</option><option>USD</option><option>EUR</option></select></label><label>Timezone<input value="Africa/Lagos" readOnly /></label><label>Weekly summary<input value="Sunday evening" readOnly /></label></div><footer>{saveMutation.error && <span>{saveMutation.error.message}</span>}<button className="button primary" type="button" disabled={saveMutation.isPending} onClick={() => saveMutation.mutate()}>{saveMutation.isPending ? 'Saving...' : 'Save settings'}</button></footer></section>
    <section className="settings-section"><header><div><p className="eyebrow">Private access</p><h2>Workspace owners</h2></div><span>{membersQuery.data?.length ?? (isPreview ? 2 : 0)} / 2 owners</span></header><form className="owner-form" onSubmit={addOwner}><div><strong>Add the second owner</strong><p>Create their Supabase Authentication user first, then enter the same email here.</p></div><input type="email" value={partnerEmail} onChange={(event) => setPartnerEmail(event.target.value)} placeholder="owner@example.com" disabled={(membersQuery.data?.length ?? 0) >= 2} /><button className="button secondary" type="submit" disabled={isPreview || ownerMutation.isPending || !partnerEmail.trim()}><UserPlus size={15} /> Add owner</button></form>{ownerMutation.error && <p className="settings-error">{ownerMutation.error.message}</p>}{ownerMutation.isSuccess && <p className="settings-success">The second owner now has access to this workspace.</p>}</section>
