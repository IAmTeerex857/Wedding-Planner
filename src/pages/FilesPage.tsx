...
export function FilesPage() {
  ...
  const filesQuery = useQuery({
    ...
    enabled: !isPreview,
    queryFn: async () => {
      const { data, error } = await supabase!.from('files').select('id,storage_path,original_name,mime_type,size_bytes,category,description,created_at').eq('workspace_id', workspace.id).is('deleted_at', null).order('created_at', { ascending: false })
  ...
  const uploadMutation = useMutation({
    mutationFn: async ({ file, title, category }: { file: File; title: string; category: string }) => {
      ...
      const { error } = await supabase!.from('files').insert({ workspace_id: workspace.id, bucket_id: 'wedding-files', storage_path: storagePath, original_name: file.name, mime_type: file.type || 'application/octet-stream', size_bytes: file.size, category, description: title || null, uploaded_by: userId, created_by: userId, updated_by: userId })
      if (error) {
...
function UploadForm({ saving, onClose, onUpload }: { saving: boolean; onClose: () => void; onUpload: (payload: { file: File; title: string; category: string }) => void }) {
  const [file, setFile] = useState<File | null>(null); const [title, setTitle] = useState(''); const [category, setCategory] = useState(categories[0])
  const [validationError, setValidationError] = useState('')
  function submit(event: FormEvent) { event.preventDefault(); if (!file) return; if (file.size > maxFileSize) { setValidationError('Choose a file smaller than 25 MB.'); return } setValidationError(''); onUpload({ file, title: title.trim(), category }) }
  return <section className="file-upload"><header><div><p className="eyebrow">Private upload</p><h2>Add a file</h2></div><button type="button" onClick={onClose}><X size={17} /></button></header><form onSubmit={submit}><label><span>Title <small>optional</small></span><input maxLength={160} value={title} onChange={(event) => setTitle(event.target.value)} /></label><label><span>Category</span><select value={category} onChange={(event) => setCategory(event.target.value)}>{categories.map((value) => <option key={value}>{value}</option>)}</select></label><label className="file-picker"><span>File</span><input type="file" required accept="image/jpeg,image/png,image/webp,application/pdf,text/csv,.xlsx" onChange={(event) => { const nextFile = event.target.files?.[0] ?? null; setFile(nextFile); setValidationError(nextFile && nextFile.size > maxFileSize ? 'Choose a file smaller than 25 MB.' : '') }} /><strong>{file?.name ?? 'Choose a file up to 25 MB'}</strong></label>{validationError && <p className="data-error file-validation-error" role="alert">{validationError}</p>}<footer><button className="button secondary" type="button" onClick={onClose}>Cancel</button><button className="button primary" type="submit" disabled={!file || Boolean(validationError) || saving}>{saving ? 'Uploading...' : 'Upload'}</button></footer></form></section>
}
