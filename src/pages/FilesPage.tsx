...
const categories = ['Photo', 'Inspiration', 'Receipt', 'Contract', 'Quote', 'Invitation', 'Travel', 'Other']
...
function UploadForm({ saving, onClose, onUpload }: { saving: boolean; onClose: () => void; onUpload: (payload: { file: File; title: string; category: string }) => void }) {
  ...
  function submit(event: FormEvent) { event.preventDefault(); if (file) onUpload({ file, title: title.trim(), category }) }
  return <section className="file-upload"><header><div><p className="eyebrow">Private upload</p><h2>Add a file</h2></div><button type="button" onClick={onClose}><X size={17} /></button></header><form onSubmit={submit}><label><span>Title <small>optional</small></span><input value={title} onChange={(event) => setTitle(event.target.value)} /></label><label><span>Category</span><select value={category} onChange={(event) => setCategory(event.target.value)}>{categories.map((value) => <option key={value}>{value}</option>)}</select></label><label className="file-picker"><span>File</span><input type="file" required accept="image/jpeg,image/png,image/webp,application/pdf,text/csv,.xlsx" onChange={(event) => setFile(event.target.files?.[0] ?? null)} /><strong>{file?.name ?? 'Choose a file up to 25 MB'}</strong></label><footer><button className="button secondary" type="button" onClick={onClose}>Cancel</button><button className="button primary" type="submit" disabled={!file || saving}>{saving ? 'Uploading...' : 'Upload'}</button></footer></form></section>
}
