...
function Registry({ title, definition }: { title: string; definition: Definition }) {
  ...
  return <div className={`page registry-page ui-page${title === 'Vendors' ? ' vendors-page' : ''}`}>
    <header className="page-header"><div><h1>{title}</h1><p className="page-lead">{definition.description}</p></div><button className="button primary" type="button" onClick={() => { setEditing(null); setAdding(true); addMutation.reset() }}><Plus size={15} /> Add {definition.noun}</button></header>
    ...
    <section className="registry-panel">
      <header><div className="registry-search-controls"><label><Search size={15} /><span className="sr-only">Search</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={`Search ${title.toLocaleLowerCase()}`} /></label>{title === 'Vendors' && <label className="vendor-category-select"><span className="sr-only">Filter vendors by category</span><Select compact aria-label="Filter by category" value={categoryFilter} onChange={setCategoryFilter} options={[{ value: 'All', label: 'All' }, ...categories.map((category) => ({ value: category, label: category }))]} /></label>}</div><span>{filtered.length} record{filtered.length === 1 ? '' : 's'}</span></header>
