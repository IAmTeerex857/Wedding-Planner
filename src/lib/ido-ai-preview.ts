...
export function previewReply(batch: IdoAiBatch, decision: 'approve' | 'reject') {
  ...
  return batch.actions[0].destination === 'vendor research'
    ? 'Search started. I will bring the results back for you to review before anything is added.'
    : `Done. ${count === 1 ? 'One change has' : `${count} changes have`} been applied to your workspace.`
}
