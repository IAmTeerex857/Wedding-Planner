...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const body = await request.json() as { workspaceId?: string; conversationId?: string; requestId?: string; content?: string; fileId?: string }
    if (!isUuid(body.workspaceId)) return response({ error: 'A valid workspaceId is required' }, 400)
    if (body.fileId !== undefined && !isUuid(body.fileId)) return response({ error: 'fileId must be a UUID' }, 400)
    let content = body.content?.trim() ?? ''
    if (!content && !body.fileId) return response({ error: 'Add a message or image' }, 400)
    if (content.length > 8_000) return response({ error: 'Message must contain no more than 8000 characters' }, 400)
    ...
    const { data: file } = body.fileId
      ? await userClient.from('files').select('id,original_name,mime_type').eq('workspace_id', body.workspaceId).eq('id', body.fileId).is('deleted_at', null).maybeSingle()
      : { data: null }
    if (body.fileId && (!file || !file.mime_type.startsWith('image/'))) return response({ error: 'Attached image not found' }, 404)
    if (!content) content = 'Analyse the attached image and propose relevant planning updates.'
    ...
    if (!existingConversation) {
      const { error: conversationError } = await admin.from('agent_conversations').insert({ id: conversationId, workspace_id: body.workspaceId, title: (file ? `Review ${file.original_name}` : content).slice(0, 80), created_by: userData.user.id, updated_by: userData.user.id })
    ...
    const { data: run, error: runError } = await admin.from('agent_runs').insert({ id: body.requestId, workspace_id: body.workspaceId, conversation_id: conversationId, status: 'queued', model: 'gpt-5.6-luna', input: { kind: 'agent_turn', file_id: file?.id ?? null }, created_by: userData.user.id }).select('id').single()
    if (runError) throw new Error('Could not queue agent run')
    const { data: message, error: insertError } = await admin.from('agent_messages').insert({ id: body.requestId, workspace_id: body.workspaceId, conversation_id: conversationId, run_id: run.id, role: 'user', content, metadata: file ? { file_id: file.id, file_name: file.original_name, mime_type: file.mime_type } : {}, created_by: userData.user.id }).select('id').single()
    if (insertError) { await admin.from('agent_runs').delete().eq('id', run.id); throw new Error('Could not queue message') }

    const triggerPayload = { workspaceId: body.workspaceId, conversationId, runId: run.id, messageId: message.id, requesterId: userData.user.id, fileId: file?.id }
